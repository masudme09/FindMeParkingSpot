defmodule Parkin.Helper.Summary do
  ...
  def summary_data(dest_lat_long, list_of_destination_lat_long) do
    list_of_destination_lat_long =
      Enum.filter(list_of_destination_lat_long, fn current_lat_long ->
        current_lat_long != dest_lat_long
      end)

    for single_lat_long_probable_destination <- list_of_destination_lat_long do
      get_zone_payment_data_from(single_lat_long_probable_destination) ++
        [
          single_lat_long_probable_destination,
          Float.to_string(
            Geolocation.probable_distance(dest_lat_long, single_lat_long_probable_destination)
          )
        ]
    end
  end
